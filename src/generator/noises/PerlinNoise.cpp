...
PerlinNoise::PerlinNoise( const std::string& _luaFile) {
...
void PerlinNoise::init( unsigned int _octaves, float _persistence, float _scale, float _maxHeight) 
{
	p = {
...
double PerlinNoise::createNoise2d(double _x, double _y)
{
	float x = _x * m_scale;
	float y = _y * m_scale;
	float total = 0.f;
	...
	for ( int i = 0; i < n ; i++ )
	{
		total += perlinNoise2d(x*frequency, y *frequency, 0.0) * amplitude;
...
double PerlinNoise::perlinNoise2d(double x, double y, double z) {
